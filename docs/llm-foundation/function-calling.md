---
title: Function Calling 与 Tool Use 协议
description: 掌握 Function Calling 的核心原理，对比 OpenAI 与 Claude 的实现差异，学习工具定义的工程最佳实践。
---
...
|------|------------------------|-----------------|
| 参数格式 | JSON 字符串（需 `JSON.parse`） | JSON 对象（直接使用） |
| 工具结果角色 | `role: "tool"` | `role: "user"` |
| 混合输出 | 调用时 `content: null` | 可同时输出文本+工具调用 |
| 强制调用 | `tool_choice: {type: "function", ...}` | `tool_choice: {type: "tool", name: "xxx"}` |

**工程最佳实践**：
1. 工具描述必须包含边界条件和取值示例
2. 必填参数用 `required` 明确标记
3. 工具数量超过 20 个时分层路由
4. 单次响应中利用并行 `tool_calls` 减少轮次

---

> 📖 **延伸阅读**
>
> 1. [OpenAI Function Calling Guide](https://platform.openai.com/docs/guides/function-calling) —— 官方完整格式规范
> 2. [Anthropic Tool Use](https://docs.anthropic.com/en/docs/build-with-claude/tool-use) —— Claude 工具调用文档
> 3. [JSON Schema](https://json-schema.org/) —— 工具参数定义的通用标准
> 4. [MCP Protocol](https://modelcontextprotocol.io/) —— 下一代工具标准化协议